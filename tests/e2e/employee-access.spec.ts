...
test.describe('Employee RBAC — projects, timeline, task sync', () => {
  ...
  test('employee sees allowed projects, timeline, and synced tasks', async ({
    ...
    await expect(
      page.locator(projectCardLink(cfg.privateProjectId))
    ).toBeVisible({
      timeout: 30_000,
    });
    await expect(
      page.locator(projectCardLink(cfg.openProjectId))
    ).toBeVisible();

    // --- Timeline board loads (standalone template) ---
    await gotoApp(page, projectCanvas(cfg.tenantId, cfg.timelineProjectId), {
      reauth,
    });
    await expect(
    ...
    await gotoApp(page, projectCanvas(cfg.tenantId, cfg.kanbanProjectId), {
      reauth,
    });
    await expect(
    ...
    await expect(
      page.getByRole('heading', { name: taskTitle, level: 4 })
    ).toBeVisible({
      timeout: 15_000,
    });
