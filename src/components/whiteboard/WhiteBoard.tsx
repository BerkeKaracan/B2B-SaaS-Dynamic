...
export default function WhiteboardBoard({
  ...
  const [strokes, setStrokes] = useState<Stroke[]>(
    (metadata.whiteboardStrokes as Stroke[]) || [],
  );
  const [texts, setTexts] = useState<FloatingText[]>(
    (metadata.whiteboardTexts as FloatingText[]) || [],
  );
  ...
  const [title, setTitle] = useState<string>(
    (metadata.whiteboardTitle as string) || "",
  );
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (activeTool === "text") {
      ...
      updateMetadata({ whiteboardTexts: updatedTexts });
      return;
    }
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    updateMetadata({ whiteboardStrokes: updatedStrokes });
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateMetadata({ whiteboardTitle: e.target.value });
  };
  ...
  const handleClearBoard = () => {
    if (
      ...
      setStrokes([]);
      setTexts([]);
      updateMetadata({ whiteboardStrokes: [], whiteboardTexts: [] });
      redrawCanvas();
    }
  };
  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-[#fdfdfc] dark:bg-[#121212] overflow-hidden select-none transition-colors duration-300">
      <div className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 shrink-0 shadow-sm flex items-center justify-between relative z-20 transition-colors duration-300">
        ...
        {/* Sağ Taraf Ekstra Araçlar ve Çöpe At Butonu */}
